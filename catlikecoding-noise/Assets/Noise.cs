...
public static class Noise
{
    ...
    {
        point *= frequency;
        int i0 = Mathf.FloorToInt(point.x) & hashMask;
        int i1 = (i0 + 1) & hashMask;

        float t = point.x - Mathf.FloorToInt(point.x);

        return Mathf.Lerp(_hash[i0], _hash[i1], Smooth(t)) * (1f / hashMask);
    }
    ...
    public static float Perlin1D(Vector3 point, float frequency)
    {
        ...
        float g0 = gradients1D[_hash[i0] & gradientsMask1D];
        float g1 = gradients1D[_hash[i1] & gradientsMask1D];
